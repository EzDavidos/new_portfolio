...
(function () {
  ...
  var FRAG = [
    ...
    '  float amp = (0.46 + 0.54 * smoothstep(0.02, 0.60, uv.x)) * guard * airTop;',
    ...
    '  float thick = dens * (0.04 + 0.34 * dens) + over * 0.50;',
    ...
    '  thick = pow(thick, 1.55) * 2.35;',

    // Красный гасится вчетверо быстрее зелёного — так вода и становится
    // бирюзовой. Синий чуть выше зелёного, иначе цвет уходит в голубой
    // и теряет зелёную ноту.
    '  vec3 ABSORB = vec3(2.35, 0.55, 0.78);',
    ...
    '  float lit = mix(1.0, 0.62 + 0.46 * diff, smoothstep(0.02, 0.35, thick));',
    ...
    '  float hi = (spec * 0.15 + spec2 * spec * 0.62) * (0.70 + 0.60 * edge);',

    '  vec3 lc = exp(-ABSORB * thick) * lit + hi + edge * 0.06;',
    ...
    // Прозрачность считается по той же толще, что и цвет, — иначе стекло
    // рассогласовано: плотность насыщалась в единицу на больших площадях и
    // они заливались ровным непрозрачным пятном. Теперь тонкое одновременно
    // и бесцветно, и прозрачно.
    '  float body = 0.08 + 0.92 * (1.0 - exp(-3.80 * thick));',
    // Дымка урезана втрое: именно она давала молочную плёнку по краям массы.
    '  float haze = shape * shape * 0.05;',

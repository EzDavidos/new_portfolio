...
(function () {
  ...
  var FRAG = [
    ...
    '  float sw = 0.20 + 0.26 * smoothstep(0.05, 0.72, uv.x);',
    '  float du = uv.y - ridge;',
    ...
    '  float lowRight = 1.0 - 0.50 * smoothstep(0.46, 0.95, uv.x) * smoothstep(0.64, 0.24, uv.y);',
    '  float amp = (0.30 + 0.70 * smoothstep(0.10, 0.72, uv.x)) * guard * airTop * lowRight;',

    '  float dv = (uv.y - ridge2) / 0.15;',
    '  float band2 = exp(-dv * dv) * (0.14 + 0.13 * smoothstep(0.10, 0.80, uv.x));',
    ...
    // Своя мера кривизны для толщи. edge = pow(1-n.z, 2.6) для этого не годится:
    // на пологой поверхности он гаснет в ноль (медиана 0.007 внутри волны), и
    // складки от него глубины не получали. Показатель 1.15 оставляет рабочий
    // диапазон. edge при этом не тронут — его использует тёмная палитра.
    '  float curv = pow(1.0 - clamp(n.z, 0.0, 1.0), 1.15);',
    ...
    // Баланс смещён с «плоской» плотности на локальные признаки: тело волны
    // от этого становится тоньше и прозрачнее, а изгибы и изнанка — заметно
    // глубже. Раньше вклад плотности задавал общий тон, и вся волна читалась
    // густой бирюзовой массой.
    '  float thick = dens * (0.02 + 0.20 * dens) + over * 0.30;',
    '  thick *= 1.0 + 6.50 * curv + 1.30 * under;',
    ...
    '  thick = pow(thick, 1.95) * 1.55;',
    // Мягкое насыщение: глубина складок сохраняется, но перестаёт расти в
    // тяжесть. Без него самые толстые места проваливались до яркости 0.18 и
    // читались густой краской, а не жидкостью.
    '  thick = thick / (1.0 + thick / 4.50);',
    ...
    '  vec3 ABSORB = vec3(2.55, 0.62, 0.66);',
    ...
    '  vec3 SCATTER = vec3(0.016, 0.130, 0.122);',
    ...
    '  float lit = mix(1.0, 0.50 + 0.52 * diff, smoothstep(0.02, 0.30, thick));',

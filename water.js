...
(function () {
  ...
  var FRAG = [
    ...
    '  float du = uv.y - ridge;',
    '  float kk = du / (du > 0.0 ? sw * 0.48 : sw * 1.35);',
    ...
    '  float airTop = 1.0 - 0.50 * smoothstep(0.78, 1.0, uv.y);',
    ...
    '  float amp = (0.30 + 0.70 * smoothstep(0.10, 0.72, uv.x)) * guard * airTop;',
    ...
    // Квадратичный рост по плотности, плюс изгибы и изнанка волны: там луч
    // идёт через материал наискось и путь длиннее всего.
    '  float thick = dens * (0.04 + 0.34 * dens) + over * 0.32;',
    '  thick *= 1.0 + 3.30 * edge + 1.05 * under;',

    // Гамма растягивает верхний хвост и поджимает нижний: тонкого материала
    // становится больше, а складки уходят заметно глубже. Без неё
    // распределение упиралось в потолок и глубоких мест просто не возникало.
    '  thick = pow(thick, 1.95) * 3.45;',

    // Красный гасится вчетверо быстрее зелёного — так вода и становится
    // бирюзовой. Зелёный и синий держатся почти вровень: это даёт teal,
    // а не зелень (синий сильно ниже) и не голубой неон (синий сильно выше).
    '  vec3 ABSORB = vec3(2.95, 0.68, 0.72);',
    ...
    '  vec3 SCATTER = vec3(0.012, 0.115, 0.108);',
    ...
    '  float lit = mix(1.0, 0.46 + 0.56 * diff, smoothstep(0.02, 0.30, thick));',

    // Блики. Слабее прежних примерно на треть и заметно уже: широкая
    // компонента срезана с 0.32 до 0.15, а вместо pow(sp,110) взята
    // pow(sp,140) — она получается умножением уже посчитанных степеней,
    // без лишнего pow. Модуляция по edge сажает отражения на изгибы
    // поверхности, а не на плоскость, поэтому больших белых пятен не
    // возникает. spec/spec2 остаются нетронутыми: их использует тёмная палитра.
    '  float hi = (spec * 0.07 + spec2 * spec * 0.42) * (0.55 + 0.65 * edge);',
    // Отражение гасится там, где материала почти нет: на плёнке блик выбивал
    // сумму за единицу и давал чистый белый. Потолок 0.30 держит его в рамках.
    '  hi *= 0.22 + 0.78 * smoothstep(0.02, 0.30, thick);',
    '  hi = min(hi, 0.30);',
    ...
    // Прозрачность держит только композиция. Раньше её приходилось лепить из
    // плотности и дымки, потому что цвет материала был белым и без гашения
    // альфой светил бы поверх страницы. Теперь прозрачность встроена в сам
    // цвет: тонкое сходится к PAPER и невидимо при любой альфе. Степень 1.25
    // подчищает края композиции.
    '  float alpha = mix(pow(clamp(shape, 0.0, 1.0), 1.25) * fade, 1.0, uDark);',
